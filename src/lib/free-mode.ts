...
export const SAFE_DELAY_OPTIONS = [30, 60, 120] as const;
...
export function getSafeDelaySec(): number {
  const v = readLocal<number>(DELAY_KEY, FREE_QUEUE_DELAY_SEC);
  return SAFE_DELAY_OPTIONS.includes(v as (typeof SAFE_DELAY_OPTIONS)[number]) ? v : FREE_QUEUE_DELAY_SEC;
}
export function useSafeDelaySec(): number {
  const v = useLocal<number>(DELAY_KEY, FREE_QUEUE_DELAY_SEC);
  return SAFE_DELAY_OPTIONS.includes(v as (typeof SAFE_DELAY_OPTIONS)[number]) ? v : FREE_QUEUE_DELAY_SEC;
}
